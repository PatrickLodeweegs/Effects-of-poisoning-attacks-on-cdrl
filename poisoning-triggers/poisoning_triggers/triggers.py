...
def select_trigger(trigger:str) -> gym.Wrapper:
    match trigger.lower():
        ...
        case "rewardinvid":
            wrapper = RewardInvId
        case "staterewardinvid":
            wrapper = StateRewardInvId
        case "reward0":
            wrapper = Reward0
        case "reward10":
            wrapper = Reward10
...
class StateRewardInvId(PoisonWrapper):
    states = [88, 52,39,76,12,72]
    @override
    def poison(self, state:Any, reward:float) -> Tuple[Any, float]:
        # print(self.obs_low, "\n", self.obs_high)
        # print(state.shape, len(self.obs_high))
        for s in StateRewardInvId.states:
            # print(f"Changing state from {state[s]}", end=" ")
            state[s] = max(self.obs_high[s] - state[s], self.obs_low[s])
            # print(f"to {state[s]}")
        return state, self.max_reward - reward